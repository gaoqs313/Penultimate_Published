...
my @G = split(/\//, $GTF);
$GTFN = $G[-1];
open(OUT1, ">StopCodonInLastExon.shorter.$GTFN");
...
open(OUT3, ">StopCodonInLastExon.shorter.$GTFN.fa");
open(OUT5, ">StopCodonInLastExon.shorter.$GTFN.aa");
open(OUT7, ">StopCodonInLastExon.shorter.$GTFN.uniq.aa");
open(OUT9, ">StopCodonInLastExon.shorter.$GTFN.uniq.filtered.aa");
...
foreach $t (keys %hash)
{
	...
	if(length($skip_aa_uniq)>$anno_aa_last_len)
	{
		$final{$t} = "";
...
open(IN1, "StopCodonInLastExon.shorter.$GTFN");
open(OUT11, ">StopCodonInLastExon.final.shorter.$GTFN");
...
while(<IN1>)
{
	...
	next if $m[$transcript_idx]  eq 'ENSMUST00000238488';

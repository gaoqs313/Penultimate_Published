...
open(OUT,">mm10.SE.gff3");
open(OUT2, ">mm10.SE.intron.bed");
open(INT, ">mm10.SE.internal.gff3");
open(OUT3, ">penultimate.novelty.txt");
open(OUT4, ">internal.novelty.txt");

open(IN, "/scratch_space/qgao/Penultimate/1.reference/mouse/E_Annotation/Internal/StopCodonInLastExon.final.internal.Mus_musculus.GRCm38.98.chr.gtf");
while(<IN>)
{
	chomp;
	@l=split(/\t/,);
...
open(IN, "/scratch_space/qgao/Penultimate/1.reference/mouse/E_Annotation/Internal/StopCodonInPenultimateExon.final.internal.Mus_musculus.GRCm38.98.chr.gtf");
while(<IN>)
{
        chomp;
        @l=split(/\t/,);

...
$dir="/scratch_space/qgao/Penultimate/6.internal/human";
open(OUT, ">human_internal_psi.txt");
...
open(REF, "/scratch_space/qgao/Penultimate/1.reference/human/B_MISO/filtered.internal.novelty.txt");
while(<REF>)
{
        chomp;
        @l = split(/\t/,);

...
## output
write.table(internal_as, "mouse_3N_support_info.txt", row.names=T, col.names=NA, quote=F, sep="\t")
...
## plot
pdf("Aggregrated_PSI_3N_distribution.pdf", width=8)
p <- ggplot(df_psi, aes(x=Annotation, y=value, fill=Annotation)) +
  geom_violin() +
  geom_boxplot(width=0.1, outlier.color=NA)+
  theme_bw() +
  facet_wrap(.~variable) +
  scale_fill_manual(name="", values=c("#B3B3B3", "#A6D854", "#E78AC3"), labels=c("All supported (5749)", "Annotated (478, 8.3%)", "Novel (5271, 91.7%)")) +
  xlab("") +

...
## plot 
## Fig 1E ##
pdf("Aggregrated_PSI_distribution.pdf", width=14)
p <- ggplot(df_psi, aes(x=Annotation, y=value, fill=Annotation)) +
  geom_violin() +
  geom_boxplot(width=0.1, outlier.color=NA)+
  theme_bw() +
  facet_wrap(.~variable, nrow=1) +
  scale_fill_manual(name="", values=c("#B3B3B3", "#A6D854", "#E78AC3"), labels=c("All supported (2152)", "Annotated (308, 14.3%)", "Novel (1844, 85.7%)")) +
  xlab("") +
  ylab("Percent Splice In") +
  theme(legend.key = element_blank(), strip.background = element_rect(fill="white"))+
  theme(axis.text.x = element_text(size = 16, family = "Helvetica")) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +

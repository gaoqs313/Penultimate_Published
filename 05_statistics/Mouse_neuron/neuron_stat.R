...
## add event id
psis <- psi
psis$Event <- rownames(psis)

## melt
penul_melt <- melt(psis)

## add tissue
penul_melt$Stage <- meta[match(penul_melt$variable, rownames(meta)), 1]

## split by tissue
penul_split <- split(penul_melt, penul_melt$Stage)
...
colnames(penul_avg) <- sort(unique(meta$Stage))
...
penul_tissue_split_number <- lapply(penul_tissue_split, function(x) apply(x[,1:8], 2, function(x) sum(x<90)))
...
bar_data$Var1 <- factor(bar_data$Var1, levels=c("DIV-8", "DIV-4", "DIV0", "DIV1", "DIV7", "DIV16", "DIV21", "DIV28"), ordered=T)

## Fig S1D ##
pdf("RNAseq_tissue_support_neuron.pdf", height=6)
p <- ggplot(bar_data, aes(x=Var1, y=value, fill=Var2)) +
        geom_bar(stat = "identity", colour="black") +
        scale_fill_manual(name = NULL, values= c("grey", "skyblue")) +
        theme_bw() +
        ylim(0, 900) +
        xlab("Neuron differentiation stage") +

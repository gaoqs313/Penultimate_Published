...
open(IN, "../A_Annotation/StopCodonInPenultimateExon.Homo_sapiens.GRCh38.98.chr.gtf.uniq.filtered.aa");
while(<IN>)
{
	chomp;
	@l = split(/\_\_/,);
...
open(IN, "../A_Annotation/StopCodonInLastExon.Homo_sapiens.GRCh38.98.chr.gtf.uniq.filtered.aa");
while(<IN>)
{
        chomp;
        @l = split(/\_\_/,);
...
open(REF, "/scratch_space/qgao/Penultimate/1.reference/human/GTF/Homo_sapiens.GRCh38.98.chr.gtf");
while(<REF>)
{
	chomp;
	next if $_=~m/^#/;
...
open(RNA, "/scratch_space/qgao/Penultimate/7.figures/human_support_info.txt");
<RNA>;
while(<RNA>)
{
	chomp;

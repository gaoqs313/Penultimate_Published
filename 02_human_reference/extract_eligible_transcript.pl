	...
	if($strand{$t} eq "+")
	{
		for($i=$numberStartCodon{$t}; $i<=$numberStopCodon{$t}; $i++)
		{
			...
			}elsif($i == $numberStopCodon{$t})
			{
				...
				$skip.= substr($fasta{$chr{$t}}, $startExon{$t}{$i}-1, $rightStopCodon{$t}-$startExon{$t}{$i}+10001);
	...
	}else
	{
                for($i=$numberStopCodon{$t}; $i>=$numberStartCodon{$t}; $i--)
                {
                        if($i == $numberStopCodon{$t})
                        {
                                ...
                                $skip.= substr($fasta{$chr{$t}}, $leftStopCodon{$t}-10001, $endExon{$t}{$i}-$leftStopCodon{$t}+10001);
...
foreach $t (keys %hash2)
{
        ...
        if($strand{$t} eq "+")
        {
				...
				$anno_utr.= substr($fasta{$chr{$t}}, $rightStopCodon{$t}, $endExon{$t}{$i}-$rightStopCodon{$t});
                        ...
                        {
                                $skip.= substr($fasta{$chr{$t}}, $startExon{$t}{$i}-1, $endExon{$t}{$i}-$startExon{$t}{$i}+10001);
        ...
        }else
        {
				...
				$anno_all+= $rightStartCodon{$t}-$startExon{$t}{$i}+1;
                        ...
                        {
                                $skip.= substr($fasta{$chr{$t}}, $startExon{$t}{$i}-10001, $endExon{$t}{$i}-$startExon{$t}{$i}+10001);
